...
DECLARE cSessions CURSOR LOCAL STATIC READ_ONLY FOR
    SELECT session_id FROM sys.dm_exec_requests WHERE command='DB STARTUP'
...
SELECT @REDOCount = COUNT(session_id) FROM sys.dm_exec_requests WHERE command='DB STARTUP'
